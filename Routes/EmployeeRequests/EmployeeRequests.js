...
router.post("/permission-requests", async (req, res) => {
  ...
  try {
    ...
    console.log(`✅ Permission request created with ID: ${result.insertId}`);
...
router.post("/meeting-requests", async (req, res) => {
  ...
  try {
    ...
    const notifiedCount = await notifySpecificUsers(attendeeIds, notificationData);

    console.log(`📢 Notified ${notifiedCount}/${attendees.length} attendees online`);
