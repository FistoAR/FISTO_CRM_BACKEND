...
router.get("/overview", async (req, res) => {
  try {
    ...
    `;
    const totalParams = employee_id ? [employee_id] : [];
    ...
    // 2. Get latest status for each client with nextFollowupDate
    const latestStatusQuery = `
      ...
      WHERE c.active = 1
      ${employee_id ? "AND f1.employee_id = ?" : ""}
    `;
    const latestParams = employee_id ? [employee_id, employee_id] : [];
    ...
    `;
    const noFollowupParams = employee_id ? [employee_id] : [];
    ...
    // Initialize counters
    let firstFollowupCount = noFollowupCount;
    ...
    const response = {
      ...
      data: {
        ...
        secondFollowup: {
          ...
          distribution: [
            { name: "Second Followup", value: secondFollowupCount },
